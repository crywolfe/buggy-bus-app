...
class Scraper
  ...
  def calculate_arrival_date
    ...
    if departure_hour > arrival_hour
      ...
      next_day = (Time.parse(@departure_date) + 84600).strftime("%a, %b %d")
  ...
  def fetch_html
    ...
    @prices = @doc.css('strong[class="cff6"]')
  end
  ...
  def get_price(entry)
    # Remove first redundant element of prices
    # with shift, select price and save to instance
    @prices.shift
